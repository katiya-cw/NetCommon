...
int NetComm(char* pServerIP, int nPort,
...
int NetSend(char *pData, int nLen)
{
	if (!g_GetData.bConnect)
		return -2;
...
int NetSendMyDataTrue2(int DtuID, unsigned char nMeter_ID, int money, int delayTime, int CH)
{
	unsigned char buff[32]; unsigned char buff3[4]; unsigned char buff4[4];
	int i;
...
int NetSendMyDataTrue3(int DtuID, unsigned char nMeter_ID, int money, int delayTime, int CH, int paymoney)
{
	unsigned char buff[32]; unsigned char buff3[4]; unsigned char buff4[4];
	int i;
...
int NetSendMyAccount(int DtuID, unsigned char nMeter_ID, int OKorNO) //对自助设备发送结算确认命令
{
	unsigned char buff[32]; unsigned char buff3[4]; unsigned char buff4[4];
	int i;
...
unsigned char MENGsendToDtu(unsigned long DevID, unsigned char Dev_id, unsigned long money, unsigned long deyTime)
{
	...
	try
	{
		i = NetSendMyDataTrue(DevID, Dev_id, money, deyTime);//向设备号为19的地址发送56.89元	           
